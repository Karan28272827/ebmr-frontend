...
export default function RetentionList() {
  ...
  const [withdrawModal, setWithdrawModal] = useState<{ open: boolean; id: string | null }>({
    open: false,
    id: null,
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      message.error('Failed to load retention samples');
    }
    setLoading(false);
  };
  ...
  const handleCreate = async (vals: any) => {
    ...
    try {
      await authApi.post('/retention', {
        ...vals,
        retain_until: vals.retain_until
          ? dayjs(vals.retain_until, 'YYYY-MM-DD').toISOString()
          : undefined,
      });
      ...
      message.error('Failed to create retention record');
    }
    setSubmitting(false);
  };
  ...
  const handleWithdraw = async (vals: any) => {
    ...
    try {
      ...
      message.error('Failed to record withdrawal');
    }
    setSubmitting(false);
  };
    ...
    },
    {
      ...
      render: (_: any, r: any) =>
        r.status !== 'WITHDRAWN' && r.status !== 'DESTROYED' ? (
          <Button size="small" onClick={() => setWithdrawModal({ open: true, id: r.id })}>
            Withdraw
          </Button>
        ) : null,
    },
  ...
  return (
    <div>
      <Typography.Title level={4}>
        <ExperimentOutlined /> Retention Samples
      </Typography.Title>

      {expiring.length > 0 && (
        <Card style={{ marginBottom: 16, borderColor: '#fa8c16' }}>
          <Typography.Text type="warning" strong>
            {expiring.length} sample(s) expiring within 60 days
          </Typography.Text>
        </Card>
      )}
      ...
      <Card>
        <Space style={{ marginBottom: 12 }}>
          <Select
            ...
            options={['STORED', 'PARTIALLY_WITHDRAWN', 'WITHDRAWN', 'DESTROYED'].map((s) => ({
              value: s,
              label: s.replace(/_/g, ' '),
            }))}
          />
      ...
      <Modal
        title="Register Retention Sample"
        open={modalOpen}
      ...
      <Modal
        ...
        onCancel={() => setWithdrawModal({ open: false, id: null })}
      ...
      >
        <Form form={withdrawForm} layout="vertical" onFinish={handleWithdraw}>
          ...
          <Form.Item name="reason" label="Reason" rules={[{ required: true }]}>
            <Select
              options={['TESTING', 'COMPLAINT_INVESTIGATION', 'REGULATORY', 'DISPOSAL'].map(
