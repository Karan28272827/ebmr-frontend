...
export default function VendorDetail() {
  ...
  const load = async () => {
    try {
      const res = await authApi.get(`/vendors/${id}`);
      setVendor(res.data);
    } catch {
  ...
  const qualify = async () => {
    try {
      await authApi.post(`/vendors/${id}/qualify`);
      message.success('Vendor qualified');
      load();
    } catch {
  ...
  const suspend = async (vals: any) => {
    try {
      await authApi.post(`/vendors/${id}/suspend`, vals);
  ...
  const poColumns = [
    ...
    { title: 'Qty', dataIndex: 'quantity', key: 'quantity' },
    {
      ...
      key: 'total_value',
      render: (v: number, r: any) => `${r.currency} ${v.toLocaleString()}`,
    },
    ...
    <div>
      ...
      {hasRole('QA_MANAGER') && (
        <Space style={{ marginBottom: 16 }}>
          {vendor.status !== 'ACTIVE' && (
            <Button type="primary" onClick={qualify}>
              Qualify Vendor
            </Button>
          )}
          {vendor.status === 'ACTIVE' && (
            <Button danger onClick={() => setSuspendModal(true)}>
              Suspend
            </Button>
          )}
        </Space>
      ...
      <Card title="Details" style={{ marginBottom: 16 }}>
        <Descriptions column={2} size="small">
          ...
          <Descriptions.Item label="Qualification Date">
            {vendor.qualification_date
              ? dayjs(vendor.qualification_date).format('DD MMM YYYY')
              : '—'}
          </Descriptions.Item>
          <Descriptions.Item label="Next Audit">
            {vendor.next_audit_date ? dayjs(vendor.next_audit_date).format('DD MMM YYYY') : '—'}
          </Descriptions.Item>
          <Descriptions.Item label="Materials">
            {(vendor.materials_supplied || []).map((m: string) => (
              <Tag key={m}>{m}</Tag>
            ))}
          </Descriptions.Item>
      ...
      <Card title="Recent Purchase Orders">
        <Table
          dataSource={vendor.purchase_orders || []}
          columns={poColumns}
          rowKey="id"
          size="small"
